...
def classification50k(data_temp, transfo_temperature, dim):
    """
    ...
    for y in range(dim[0]):  # y
        for x in range(dim[1]):  # x
            if data_temp.iloc[1936 * y + x, 2] >= 1475:
                res[y][x] = transfo_temperature[-1][1]

            elif data_temp.iloc[1936 * y + x, 2] < 275:
                res[y][x] = [255, 255, 255]
...
def mask50k(transfo_temperature, dim, classification):
    """
    Classifies pixels into temperature groups and assigns them one color per class.
